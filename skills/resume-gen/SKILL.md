...
3. 读取对应 `experiences/` 目录、项目经历、校园经历作为增强资料（legacy: `data/experiences/`）
...
`experiences/`、项目经历、校园经历为空时不阻断，但应提示用户补充可显著提升匹配质量。
...
读取 `experiences/*.md`（跳过 `_template.md`、`README.md`、空文件），同时读取 `profile.md` 中的项目经历和校园经历。
...
| 校园经历 | **0-2 段** | 仅在 JD 相关、经历不足或能证明组织/领导/活动运营/竞赛成果时加入 |
| **合计** | **≤ 5 段** | 目标占满整页 |
...
**数量**：实习/工作经历每段 2-3 条，最多 4 条；项目/研究/校园经历 1-2 条
...
| 校园经历 / Campus Experience | Step 2 选定的校园经历 | 可选，0-2 段，不相关时不展示 |
